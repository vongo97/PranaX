...
def generate_brown_noise(filename, duration_sec=30.0, sample_rate=44100):
    # Ruido marrón (integral del ruido blanco) - Suena a viento / hojas / mar relajante
    num_samples = int(duration_sec * sample_rate)
    filepath = os.path.join(RAW_DIR, filename)
    ...
    with wave.open(filepath, 'w') as wav_file:
        ...
        for i in range(num_samples):
            ...
            last_val = (last_val + 0.02 * white) / 1.02 # Filtro pasa-bajos simple
            
            # Fade in and fade out para loop perfecto
            t = float(i) / sample_rate
            envelope = 1.0
            if t < 2.0: envelope = t / 2.0
            elif t > duration_sec - 2.0: envelope = (duration_sec - t) / 2.0
                
            sample = last_val * 4.0 * envelope # Boost volume
            sample = max(-1.0, min(1.0, sample))
            int_sample = int(sample * 32767.0)
            data = struct.pack('<h', int_sample)
            wav_file.writeframesraw(data)
...
print("Generando Campana de Cristal (Campana Acústica Corta)...")
...
print("Generando Sonido de Viento/Bosque (Ruido Marrón)...")
